...
impl Position{
    ...
    pub fn apply_aimed_mvt(&mut self, mvt : Movement) {
        match mvt {
            Forward(val) => { self.horz += val;self.depth += self.aim * val },
...
fn parse_movement(line :&str) -> Option<Movement> {
    ...
    instructions.map(|(direct, val)|{
        val.parse::<usize>().ok().map(|val|{
            match direct {
...
fn apply_plan(init_pos : &Position, plan :&str ) -> Position {
    let movements : Vec<_>= plan.lines()
        .filter_map(|line| parse_movement(line))
        .collect();
...
pub fn print_position() {
    let plan = include_str!("day2_movements.txt");
...
forward 2
        ";
        assert_eq!(Position{horz:15,depth:10, aim:0}, apply_plan(&init_pos, plan));
