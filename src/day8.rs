...
impl DisplaysSource {
    ...
    pub fn decode(&self) -> usize {
        ...
        for _ in 0..10 {
            // max 10 iterrations since there are 10 unknown
            ...
            let known_digits: Vec<_> = digit_to_source_wires
                ...
                .collect();
            let almost_known_digits = digit_to_source_wires
                .iter()
                .filter(|(d, wires)| wires.len() + 1 == get_wires(**d).len());
            dbg!(known_digits.len());
            dbg!(almost_known_digits.count());
